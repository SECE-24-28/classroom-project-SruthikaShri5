            ...
            <div className="flex flex-col sm:flex-row gap-6 justify-center items-center">
              {!user ? (
                <>
                  <Link
                    to="/signup"
                    className="btn-emerald px-6 sm:px-10 py-4 sm:py-5 text-lg sm:text-xl font-semibold rounded-2xl shadow-2xl hover:shadow-3xl transform hover:-translate-y-2 transition-all duration-500 flex items-center justify-center w-full sm:w-auto"
                  >
                    Start Free Today
                    <FiArrowRight className="ml-3 w-5 h-5 sm:w-6 sm:h-6" />
                  </Link>
                  <Link
                    to="/plans"
                    className="btn-cyan px-6 sm:px-10 py-4 sm:py-5 text-lg sm:text-xl font-semibold rounded-2xl transition-all duration-500 flex items-center justify-center w-full sm:w-auto"
                  >
                    Explore Plans
                    <FiStar className="ml-3 w-5 h-5 sm:w-6 sm:h-6" />
                  </Link>
                </>
              ) : (
                <>
                  <Link
                    to="/recharge"
                    className="btn-emerald px-10 py-5 text-xl font-semibold rounded-2xl shadow-2xl hover:shadow-3xl transform hover:-translate-y-2 transition-all duration-500 flex items-center"
                  >
                    Recharge Now
                    <FiZap className="ml-3 w-6 h-6" />
                  </Link>
                  <Link
                    to="/dashboard"
                    className="btn-cyan px-10 py-5 text-xl font-semibold rounded-2xl transition-all duration-500 flex items-center"
                  >
                    My Dashboard
                    <FiTrendingUp className="ml-3 w-6 h-6" />
                  </Link>
                </>
              )}
            </div>
            ...
            {stats.map((stat, index) => (
              <div key={index} className="text-center animate-scale-in card p-8 hover:shadow-2xl" style={{animationDelay: `${index * 0.2}s`}}>
                <div className="flex justify-center mb-6">
                  <div className={`p-4 bg-gradient-to-r from-gray-100 to-gray-50 rounded-2xl ${stat.color}`}>
                    {stat.icon}
                  </div>
                </div>
              ...
              >
                <div className="relative z-10">
                  ...
                  <div className="flex items-center mb-4 sm:mb-6">
                    <div className="p-2 sm:p-3 bg-white rounded-xl shadow-lg mr-3 sm:mr-4">
                      {feature.icon}
                    </div>
                    <h3 className="text-xl sm:text-2xl font-bold font-roboto text-gray-900">{feature.title}</h3>
                  </div>
                  <p className="text-gray-600 leading-relaxed font-opensans text-base sm:text-lg">{feature.description}</p>
                </div>
                <div className="overlay-content">
                  <FiHeart className="w-12 h-12 mb-4 mx-auto" />
        ...
        <div className="relative max-w-5xl mx-auto text-center px-4 sm:px-6 lg:px-8 animate-fade-in">
          ...
          {!user ? (
            <div className="flex flex-col sm:flex-row gap-6 justify-center">
              <Link
                to="/signup"
                className="inline-flex items-center px-10 py-5 bg-white text-emerald-600 font-bold font-roboto text-xl rounded-2xl hover:bg-gray-100 transition-all duration-500 shadow-2xl hover:shadow-3xl transform hover:-translate-y-2"
              >
                Begin Your Journey
              ...
              <Link
                to="/plans"
                className="inline-flex items-center px-10 py-5 border-3 border-white text-white font-bold font-roboto text-xl rounded-2xl hover:bg-white hover:text-emerald-600 transition-all duration-500"
              >
                View Premium Plans
                <FiStar className="ml-3 w-6 h-6" />
              </Link>
            </div>
          ) : (
            <Link
